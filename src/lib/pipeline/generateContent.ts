...
import { getVoiceoverText, parseSubtitles, subtitleTextForTts } from "../media/subtitles";
  ...
  try {
    ...
    await prisma.generatedScript.create({
      data: {
        ...
        shortSummary: scriptResult.shortSummary,
...
export async function runGenerateVideo(
  ...
  const article = await prisma.article.findUnique({
    where: { id: articleId },
    include: { script: true, video: true, source: true },
  });
  ...
  if (article.video && !options?.regenerate) {
    return {
      ...
      sourceVideoUsed: article.video.sourceVideoUsed,
  ...
  try {
    ...
    const subtitles = parseSubtitles(article.script.subtitles);
    const narration =
      subtitles.length > 0
        ? subtitleTextForTts(subtitles)
        : getVoiceoverText(article.script);

    console.log(`[runGenerateVideo] Generating Romanian voice-over articleId=${articleId}`);
    ...
    await prisma.generatedScript.update({
      where: { articleId },
      data: { voiceoverSkipped: voiceover.skipped, voiceoverScript: narration },
    });
    ...
    await prisma.generatedVideo.create({
      data: {
        ...
        sourceVideoUsed: videoResult.sourceVideoUsed,
    ...
    console.log(
      `[runGenerateVideo] Done articleId=${articleId} images=${videoResult.imageCount} ` +
        `sourceVideo=${videoResult.sourceVideoUsed} duration=${videoResult.duration}s`
    );
